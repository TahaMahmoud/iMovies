...
final class GetTopRatedUseCaseTests: XCTestCase {
    var moviesRepoSuccessMock: MoviesRepoProtocol!
    var moviesRepoFailedMock: MoviesRepoProtocol!

    var getTopRatedUseCase: GetMoviesBaseUseCaseProtocol!
    ...
    func testGetTopRatedSuccess() async throws {
        getTopRatedUseCase = GetTopRatedUseCase(repo: moviesRepoSuccessMock)

        let input = GetMoviesUseCaseInput(page: 1)

        let result = await getTopRatedUseCase.execute(input)
